...
func (s *streamFrameSorter) OutofOrderBytes() protocol.ByteCount {
	var ofobytes protocol.ByteCount
	var last_end protocol.ByteCount
	var gap *utils.ByteIntervalElement
	for gap = s.gaps.Front(); gap != nil; gap = gap.Next() {
		if last_end == 0 {
			last_end = gap.Value.End
		} else {
			ofobytes += gap.Value.Start - last_end
			last_end = gap.Value.End
		}
	}
	return ofobytes
