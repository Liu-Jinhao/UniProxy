...
type stream struct {
	...
	// token represents the number of allowed packets to be sent
	token int
	// every stream is allocated to a path
	path *path
	// the packets sent on every path since last allocation
	packetSent map[protocol.PathID]int
	// the file descriptor of the corresponding TCP socket
	fd int
	noDataTime time.Time

	// use of bytes and ofo stats
	// byteSent map[protocol.PathID]protocol.ByteCount
	// openTime time.Time
	// startStat bool
	// endStat bool
	// ofoBytes protocol.ByteCount
	// avgOfoBytes float32
	// ofoTime time.Time
...
func newStream(StreamID protocol.StreamID,
	...
	s := &stream{
		...
		packetSent:			make(map[protocol.PathID]int),
		noDataTime:			time.Now(),
		// byteSent:			make(map[protocol.PathID]protocol.ByteCount),
		// openTime:			time.Now(),
		// startStat:			false,
		// endStat:			false,
		// ofoTime:			time.Now(),
...
func (s *stream) hasUnsentData() bool {
	if s.lenOfDataForWriting() == 0 {
		if s.fd == 0 {
			return false
		}

		available, err := unix.IoctlGetInt(s.fd, 0x541B)
...
func (s *stream) SetTCPConn(conn *net.Conn) {
...
func (s *stream) AddStreamFrame(frame *wire.StreamFrame) error {
	...
	// now := time.Now()
	// if s.ofoBytes != 0 {
	// 	s.avgOfoBytes += float32(now.Sub(s.ofoTime))/float32(time.Second)*float32(s.ofoBytes)
	// }
	// s.ofoBytes = s.frameQueue.OutofOrderBytes()
	// s.ofoTime = now
	...
	// 	utils.Infof("stream %v avg ofobytes %v", s.streamID, s.avgOfoBytes/60)
	// 	s.endStat = true
	// }
