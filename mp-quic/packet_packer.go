...
// StoreFrames compose a packet but not actually Seal it. The frames are stored.
...
func (p *packetPacker) PackPacketWithStoreFrames(pth *path) (*packedPacket, error) {
	...
	if p.streamFramer.HasCryptoStreamFrame() {
		return p.packCryptoPacket(pth) // if stream 1 has packets
	}

	encLevel, sealer := p.cryptoSetup.GetSealer()

	publicHeader := p.getPublicHeader(encLevel, pth)
	publicHeaderLength, err := publicHeader.GetLength(p.perspective) // max headerlen = 20
	if err != nil {
		return nil, err
	}
	p.stopWaiting[pth.pathID] = nil
