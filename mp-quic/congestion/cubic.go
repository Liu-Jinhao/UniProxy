...
func (c *Cubic) alpha() float32 {
	...
	b := c.beta()
	return 3 * c.numConnections * c.numConnections * (1 - b) / (1 + b)
}
...
func (c *Cubic) beta() float32 {
	...
	// computed as:
	return (c.numConnections - 1 + beta) / c.numConnections
}
...
func (c *Cubic) betaLastMax() float32 {
	...
	return (c.numConnections - 1 + betaLastMax) / c.numConnections
}
...
func (c *Cubic) SetNumConnections(n float32) {
	c.numConnections = n
}
