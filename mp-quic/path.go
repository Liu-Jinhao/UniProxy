...
type path struct {
	...
	congestionlimited time.Time
	bandwidthshared time.Time
}
...
func (p *path) setup(oliaSenders map[protocol.PathID]*congestion.OliaSender) {
	...
	sentPacketHandler := ackhandler.NewSentPacketHandler(p.rttStats, nil, p.onRTO)
	...
	p.congestionlimited = time.Now()
	p.bandwidthshared = time.Now()
